            ...
            for columna in range(len(self.matriz[linea])):
                if not self.matriz[linea][columna] is None:
                    ...
                    if nombre == instancia.get():
                        respuesta = instancia.operaciones(parametro1, parametro2)
                        print(respuesta)
                        existe = True
                        break
        return respuesta
...
#------------------------------Fin de accionar los conocimientos de la memoria-----------------------------------------#
    ...
    def aprender_externo(self, nombre, codigo):
        return self.creacion_de_memoria.aprender(Operaciones.funcion_externa(nombre, codigo))
